...
export default function AddBike() {
  ...
  const [form, setForm] = useState({ type: '', model: '', manufacturer: '', year: '', imageUrl: '',
    driveBrand:'', driveType:'', color:'', brakes:'', suspension:'', suspensionType:'', specs:'' });
      ...
      <div className="addbike-white-card">
        ...
        <div className="ab-inner-panel">
          <div className="ab-form-col">
            {step === 1 && (
              <>
                ...
                <form id="addbike-form" onSubmit={handleSubmit} className="ab-form" autoComplete="off">
                  ...
                  <label className="ab-field">
                    <span>Model</span>
                    <input placeholder="Např. MX 7206P" value={form.model} onChange={e => update('model', e.target.value)} />
                  </label>
                  <label className="ab-field">
                    <span>Výrobce</span>
                    <input placeholder="Např. Maxbike" value={form.manufacturer} onChange={e => update('manufacturer', e.target.value)} />
                  </label>
                  <label className="ab-field">
                    <span>Rok výroby</span>
                    <input type="number" placeholder={`Např. ${currentYear}`} value={form.year} onChange={e => update('year', e.target.value)} />
                  </label>
              ...
              <>
                ...
                <form id="addbike-form" onSubmit={handleSubmit} className="ab-form" autoComplete="off">
                  <label className="ab-field">
                    ...
                    <input value={form.driveBrand} onChange={e => update('driveBrand', e.target.value)} />
                  </label>
                  <label className="ab-field">
                  ...
                  <label className="ab-field">
                    <span>Odpružení</span>
                    <input value={form.suspension} onChange={e => update('suspension', e.target.value)} />
                  </label>
                  <label className="ab-field">
                    <span>Typ Odpružení</span>
                    <input value={form.suspensionType} onChange={e => update('suspensionType', e.target.value)} />
                  </label>
                  <label className="ab-field">
          ...
          <div className="ab-image-col">
            <div className="ab-image-frame large">
              {preview ? <img src={preview} alt="Náhled kola" /> : <div className="ab-image-placeholder">Žádný obrázek</div>}
            </div>
            <div className="ab-upload-box overlay">
              <div>{preview ? 'Aktuální fotka' : 'Přidejte fotku vašeho kola'}</div>
              <input id="bike-image-input" type="file" accept="image/*" style={{ display: 'none' }} onChange={e => handleImageFile(e.target.files?.[0])} />
              <div style={{ display:'flex', gap:8 }}>
                <button type="button" onClick={() => document.getElementById('bike-image-input').click()} className="ab-upload-btn">{preview ? 'Změnit' : 'Nahrát'}</button>
                {preview && (
                  <button type="button" className="ab-remove-btn" onClick={() => { setPreview(''); update('imageUrl',''); }}>Odstranit</button>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="ab-bottom-bar">
          <div className="ab-bottom-left" style={{ display:'flex', gap:16 }}>
            <button type="button" className="ab-continue ab-back-btn" onClick={() => { if (step === 2) { setStep(1); setError(''); } else { navigate(-1); } }}>
              <span>Zpět</span>
              <span className="ab-continue-icon" style={{ transform:'rotate(180deg)' }}>➜</span>
            </button>
            <button type="button" className="ab-continue ab-cancel-btn" onClick={() => navigate('/my-bikes')}>
              <span>Zrušit</span>
