...
router.get('/users', requireAuth, requireAdmin, async (req, res) => {
  try {
    const users = await User.find({}, { email: 1, role: 1, isVerified: 1, createdAt: 1 }).sort({ createdAt: -1 }).lean();
...
router.post('/users/:id/role',
  ...
  [
    param('id').isMongoId(),
    body('role').isIn(['user','admin']).withMessage('Neplatná role')
  ],
  async (req, res) => {
    if (!handleValidation(req, res)) return;
    try {
      if (String(req.user.id) === req.params.id && req.body.role !== 'admin') {
        return res.status(400).json({ error: 'Nelze degradovat sám sebe.' });
      }
